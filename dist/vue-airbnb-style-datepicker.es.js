...
var AirbnbStyleDatepicker = {
  ...
  computed: {
    ...
    availability: function () {
      return this.$store.getters['property/getAvaibility']
    }
  },
  watch: {
  ...
  methods: {
    setEnableDates: function () {
      for (let i = 0; i < this.availability.length; i++) {
        if (this.availability[i].availability) {
          this.enabledDates.push(this.availability[i].date)
        } else {
          this.disabledDates.push(this.availability[i].date)
        }
      };
    },
    ...
    handleKeyboardInput: function handleKeyboardInput(event) {
      if (this.shouldHandleInput(event, this.keys.esc)) {
        ...
        if (!this.showKeyboardShortcutsMenu && target && target.tagName === 'TD') {
          this.onSelectDayClickHandler(this.focusedDate);
        }
    ...
    setDateFromText: function setDateFromText(value) {
      ...
      this.generateMonths();
      this.generateYears();
      this.onSelectDayClickHandler(formattedDate);
    },
    ...
    setupDatepicker: function setupDatepicker() {
      ...
      const { apply, cancel } = this.locales[this.$store.$i18n.locale].texts;
      this.texts = {};
      this.texts.apply = apply;
      this.texts.cancel = cancel;
    ...
    onSelectDayClickHandler: function onSelectDayClickHandler(date) {
      ...
      if (this.isSelectingDate1 || isBefore(date, this.selectedDate1)) {
        ...
        let differenceInDays = this.getDifferenceInDays(this.selectedDate1, this.selectedDate2);
        if (differenceInDays < this.minStay) {
          differenceInDays = this.minStay - differenceInDays;
          this.selectedDate2 = this.addDays(this.selectedDate2, differenceInDays);
        };

        this.enabledMinStayDays();
    ...
    addDays: function (date, days) {
      ...
      var currentDate = new Date(dateTo.valueOf());
    ...
    setMinStay: function setMinStay() {
      ...
      this.minStay = selectedDateData.length > 0 && selectedDateData[0]['min_stay'] ? selectedDateData[0]['min_stay'] : 5;
    },
    ...
    getDifferenceInDays(date1, date2) {
      // Utils method to calculate difference in days between two dates
    ...
    isDateDisabled: function isDateDisabled(date) {
      ...
      /*------------------------------ Customized method to handle days with off availability ------------------------------------ */

      return this.disabledDates.includes(date) ? true : false;
    ...
    customizedDateClass: function customizedDateClass(date) {
      ...
      if (this.customizedDates.length > 0) {
        for (var i = 0; i < this.customizedDates.length; i++) {
          if (this$1.customizedDates[i].dates.indexOf(date) > -1) { customizedClasses += " asd__day--" + (this$1.customizedDates[i].cssClass); }
        }
      }
